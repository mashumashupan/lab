...
public abstract class GraphLayout<VTYPE extends GraphLayoutVertex> {
	...
	public GraphLayout( Graph _g, ArrayList<double[]> positions){
		if (positions == null || positions.size() == 0) {
			throw new IllegalArgumentException("positions is null or empty");
		}
		w = positions.stream().map(p -> p[0]).max(Double::compare).get().intValue();
		h = positions.stream().map(p -> p[1]).max(Double::compare).get().intValue();

		System.out.println("positions = " + positions.size());
		System.out.println("nodes = " + _g.getNodeCount());
		for( int i = 0; i < _g.getNodeCount(); i++){
			float x = (float) positions.get(i)[0];
			float y = (float) positions.get(i)[1];
			VTYPE v = createVertex( _g.getVertex(i), x, y );
			layoutVerts.add( v );
			graph2layout.put( _g.getVertex(i), v );
		}
	}
